...
| a hostile or broken package reaches your home: a maintainer script writing `~/.bashrc` or reading `~/.ssh`, a file shipped through another package's symlink, a `../` member | done | the install view hides your home but for the prefix, and `/tmp`; `dev/hostile-debs.sh` checks it | [`view.md`](view.md) |
