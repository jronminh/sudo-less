...
//
// Transport: a unix stream socket, /run/updo/IDENT.sock, one daemon instance
...
// re-checks the caller's uid with SO_PEERCRED against updo.conf before
// reading anything.
//
...
//      cmd    argv is the command                           updo CMD
//      shell  interactive shell, or argv through the shell  updo, updo -s
//      login  the same as a login shell                     updo -i
//      line   argv[0] is a shell command line               updo -c, -f
//      list   describe the identity (from updo.conf)        updo -l
//      which  where argv[0] resolves, and if it is allowed  updo -l CMD
//      read   copy file argv[0] to stdout                   updo -e
//      write  replace the contents of file argv[0] by stdin updo -e
